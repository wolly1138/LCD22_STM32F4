...
{
	alt_u8 n,m,tmp;
	if(touch_counter>=(TOUCH_MAX_CACHE-1)) {return 0;}
	...
	alt_u32 tmp_x = ((alt_u32)tmp_xy_buf[SAMP_COUNT/2].x + (alt_u32)tmp_xy_buf[SAMP_COUNT/2-1].x)/2;
	alt_u32 tmp_y = ((alt_u32)tmp_xy_buf[SAMP_COUNT/2].y + (alt_u32)tmp_xy_buf[SAMP_COUNT/2-1].y)/2;

	if(tmp_x >= 0xFFF || tmp_y >= 0xFFF) {return 0;}
	touch_xy_buffer[touch_wr_index].x = ((tmp_x * 240)>>12);
	touch_xy_buffer[touch_wr_index].y = ((tmp_y * 320)>>12);
	if(touch_wr_index < (TOUCH_MAX_CACHE-1)) {touch_wr_index++;}
	else {touch_wr_index = 0;}
	touch_counter++;
	return 1;
}
...
alt_u8 draw_lcd(void)
{
	...
	if(touch_rd_index < (TOUCH_MAX_CACHE-1)) {touch_rd_index++;}
	else {touch_rd_index = 0;}
